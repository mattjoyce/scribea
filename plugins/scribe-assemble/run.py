...
Pure function of (clips in seq order, template_id, transcript source,
latest EMR Backstory). Re-running on the same input must produce byte-
identical assembled_context — that's the cache key for the structure-
worker's llm_cache.
...
Per ADR-0006, when a session has a scribe.case.context_attached.v1 event
the assembled context begins with an EMR Backstory block:
...
Without that event the EMR Backstory section is omitted and only the
transcript section is produced (preserving the session-without-context
flow unchanged).
"""
...
def compose(session: dict, clips: list[dict], template_id: str, *,
            source: str, blobs_dir: str,
            backstory_lines: list[str] | None = None) -> tuple[str, list[dict], str]:
...
def main() -> None:
    ...
    assembled_context, gaps, effective_source = compose(
        ...
        backstory_lines=backstory_lines,
    )
    sw.mark("compose_ms")
    try:
